...
describe("Negative Tests", function () {
    ...
    it("Asset transfer fails when supply is insufficient" , () => {
       ...
       // do opt in
       commonfn.optIn(runtime, master.account, appInfoHolding.appID, assetID);
    ...
    it("Asset burn fails when supply is insufficient" , () => {
        ...
        // do opt in
        commonfn.optIn(runtime, master.account, appInfoBurn.appID, assetID);
    ...
    it("Selling token fails when supply < amount solde" , () => {
        appInfoMint = commonfn.initMint(runtime,master.account);
        const assetID = commonfn.createAsset(runtime,master.account,appInfoMint.appID);
        const appInfoHolding = commonfn.initHolding(runtime,master.account,assetID);
        const appInfoBurn = commonfn.initBurn(runtime,master.account,assetID);
        ...
        assert.throws(() =>{commonfn.selling(runtime, acc1.account, assetID, appInfoHolding.applicationAccount,amountOfAlgo, appInfoHolding.appID,amountOfAsset)}, RUNTIME_ERR1009);
    }).timeout(10000);

...
def mint_approval():
    ...
    asset_transfer_holding = Seq([
        Assert(basic_checks),
        Assert(App.globalGet(Bytes("holdingAddress")) == Tnx.accounts[1])
    ...
    asset_transfer_burn = Seq([
        Assert(basic_checks),
        Assert(App.globalGet(Bytes("burnAddress")) == Tnx.accounts[1])
        Assert(App.globalGet(Bytes("teslaid"))==Txn.assets[0]),
        Assert(Txn.sender() == Global.creator_address()), # creator only function
    ...
    save = Seq(
        App.globalPut(Bytes("holdingAddress"), Txn.accounts[1]),
        App.globalPut(Bytes("burnAddress"), Txn.accounts[2]),
        Return(Int(1)),
    )
    ...
    handle_noop = Seq(
        Cond(
            ...
            [Txn.application_args[0] == Bytes("save"), save],
        )
    )
